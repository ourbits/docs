...
- 接口功能：删除指定种子
- 接口请求动作：`POST torrent/delete/{id:\d+}`
- 接口请求参数：

|    参数名     | 必要 |              类型              |    默认值    | 说明                                                  |
|:----------:|:--:|:----------------------------:|:---------:|:----------------------------------------------------|
|   value    | x  | enum{ 'torrent', 'trailer' } | 'torrent' | 删除类型（因历史原因，传入 trailer 可以删除预告种子                      |
| reasontype | x  |      enum{0,1,2,3,4,5}       |     0     | 删除的原因类型，0 - 其他，1 - 断种，2 - 重复，3 - 劣质，4 - 单集回收，5 - 违规 | 
|   reason   | ?  |            string            |    ""     | 删除的原因描述（系统会使用 reasontype 和 reason 拼成最终的删种原因）        |
